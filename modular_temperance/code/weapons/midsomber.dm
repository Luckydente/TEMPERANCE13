...
/obj/item/rogueweapon/sword/sabre/midsomber/ChargeAttack(target, mob/living/carbon/human/user)
	..()
	if(!user)
		return
	RegisterSignal(user, COMSIG_CLICK_CTRL, PROC_REF(try_furioso))
	src.visible_message(span_boldwarning("<b>[user]</b> coagulates their gathered blood!"))
...
/obj/item/rogueweapon/sword/sabre/midsomber/proc/furioso_flurry(mob/living/user, mob/living/target, list/weapon_data, list/colors)
...
/obj/item/rogueweapon/sword/sabre/midsomber/proc/furioso_circle(mob/living/user, mob/living/target, list/weapon_data, list/colors)
	var/beam_color = colors[weapon_data["damtype"]]
...
/obj/item/rogueweapon/sword/sabre/midsomber/proc/furioso_dashthrough(mob/living/user, mob/living/target, list/weapon_data, list/colors, heavy)
	var/beam_color = colors[weapon_data["damtype"]]
...
/obj/item/rogueweapon/sword/sabre/midsomber/proc/furioso_lunge(mob/living/user, mob/living/target, list/weapon_data, list/colors)
	var/beam_color = colors[weapon_data["damtype"]]
	for(var/i in 1 to weapon_data["hits"])
		if(QDELETED(src) || QDELETED(user) || QDELETED(target))
			return
...
/obj/item/rogueweapon/sword/sabre/midsomber/proc/furioso_charge(mob/living/user, mob/living/target, list/weapon_data, list/colors)
	var/beam_color = colors[weapon_data["damtype"]]
	for(var/i in 1 to weapon_data["hits"])
		if(QDELETED(src) || QDELETED(user) || QDELETED(target))
			return
...
/obj/item/rogueweapon/sword/sabre/midsomber/proc/furioso_ranged(mob/living/user, mob/living/target, list/weapon_data, list/colors)
	var/beam_color = colors[weapon_data["damtype"]]
...
/obj/item/rogueweapon/sword/sabre/midsomber/proc/furioso_end(mob/living/user, list/targets)
	...
	to_chat(user, span_notice("Furioso complete. The lance returns to its inactive state."))
	qdel(src)
